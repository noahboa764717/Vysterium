...
function activeTabChange (data) {
  ...
  if (iframe.src != location.protocol + "//" + location.hostname +'/newtab.html' && iframe.src.indexOf(index$config.prefix) > -1){
    // let link = _browser_.decodeUrl(iframe.contentWindow.location.href.split("/service/")[1])
  adr.innerText = _browser_.decodeUrl(iframe.contentWindow.location.href.split(index$config.prefix)[1])
...
//  var surelol = input.split("genow://")[1]
  ...
  }else if (iframe.src == './newtab.html') {
    adr.innerHTML = ''
  } else {
    adr.innerHTML = 'undef'
  }
