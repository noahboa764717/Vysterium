...
function ser() { 
    ...
    window.open(selfindex$config.prefix + selfindex$config.encodeUrl(url), "_self");
}
...
if (urltoOpen == null) {
    siteFrame.setAttribute("src", `https://${parentDomain}' + selfindex$config.prefix + ${selfindex$config.encodeUrl(urlToOpen)}`);
...
if (urlToOpen != "undefined") {
    ...
    if(parentDomain === "localhost") {
        siteFrame.setAttribute("src", `//${parentDomain}:${window.parent.document.location.port}`  + selfindex$config.prefix + `${selfindex$config.encodeUrl(urlToOpen)}`);
        urlbar.value = urlToOpen;
    }
}