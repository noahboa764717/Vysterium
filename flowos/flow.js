	...
	boot = async () => {
		...
		_auth.onAuthStateChanged(async (user) => {
			...
			new WinBox({
				...
				y: 'center',
				height: '650px',
				html: `<iframe src="/flowos/builtin/apps/setup.html" title="Setup Wizard" scrolling="yes"></iframe>`,
			});
			this.setup = true;
		});
	};
	...
	apps = {
		register: () => {
			for (const [APP_ID, value] of Object.entries(apps())) {
				...
				appListItem.innerHTML = `<img src="/flowos/assets/icons/${APP_ID}.svg" width="25px"/>${value.title}`;
				appListItem.onclick = () => {
		...
		open: (APP_ID) => {
			...
			url = apps()[APP_ID].proxy == false ? apps()[APP_ID].url : `${__uv$config.prefix}${__uv$config.encodeUrl(apps()[APP_ID].url)}`;
